...
def check_btc_4h_signal():
    """
    Evaluates BTC/USDT 4H candles (limit=1000) against Kivanc Pine Script v4 Supertrend.
    Fast ST: calculate_kivanc_supertrend(df, period=10, multiplier=3.0)
    Slow ST: calculate_kivanc_supertrend(df, period=12, multiplier=10.0)
    ...
    # 1. HỖ TRỢ (🔵): When Dải 1 is Uptrend (1) & distance <= 0.8%
    if trend1 == 1:
        diff_blue = ((current_price - val1) / current_price) * 100.0
        if 0.0 <= diff_blue <= 0.8:
            val_blue_str = format_price_level(val1)
            val_green_str = format_price_level(val_green)
            gap_green = ((val1 - val_green) / val1) * 100.0 if val1 > 0 else 0.0
    ...
    # 2. CẢN (🔴): When Dải 1 is Downtrend (-1) & distance <= 0.8%
    if trend1 == -1:
        diff_red = ((val1 - current_price) / current_price) * 100.0
        if 0.0 <= diff_red <= 0.8:
            val_red_str = format_price_level(val1)
            val_yellow_str = format_price_level(val_yellow)
            gap_yellow = ((val_yellow - val1) / val1) * 100.0 if val1 > 0 else 0.0
            ...
            alert_msg = (
                ...
                f"▫️ <b>Giá hiện tại:</b> {price_str} USDT\n"
                f"▫️ <b>Mốc Red 🔴:</b> {val_red_str} (cách {diff_red:.2f}%)\n"
...
def get_btc_4h_check_report():
    ...
    Prints current BTC price and exact values of both Supertrend bands.
    """
    ticker = get_ticker_24h("BTCUSDT")
    df = get_klines_df("BTCUSDT", "4h", 1000)
    ...
    if not ticker or candle_count < 30:
        return "❌ Không thể kết nối tới Binance Spot API để lấy nến BTC 4H."
    ...
    report = [
        ...
        f"{status_note}\n",
        "✅ <i>Đối chiếu 100% khớp từng con số với biểu đồ TradingView Kivanc Supertrend!</i>"
    ]
    return "\n".join(report)
