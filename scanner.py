...
from ta_engine import get_trend_status
...
def get_coin_report(symbol="BTC"):
    """
    Pure Trend Scan Report (30M, 1H, 2H, 4H, 8H, 12H, 1D, 1W, 1M).
    Formats each timeframe:
    ▫️ {TF}: {Trend_Status}
    """
    ...
    report = []
    report.append(f"🪙 **{symbol_upper}/USDT** (TREND SCAN)\n")

    for tf in SCAN_TIMEFRAMES:
        candles = klines_map.get(tf, [])
        trend_st = get_trend_status(candles)
        tf_label = tf.upper()
...
def scan_market(coins_list=None):
    """
    Market Watchlist Scanner using Pure Trend Scan.
    """
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in scan_futures:
            res = future.result()
            if res and "TREND SCAN" in res:
                results.append(res)

    if not results:
        return "⚪ Không thể lấy báo cáo quét Trend."
