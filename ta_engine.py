...
def calculate_supertrend(candles, atr_period=10, multiplier=3.0):
    ...
    if len(candles) <= atr_period + 5:
        return {"trend": "NEUTRAL", "is_buy": False, "value": None, "is_flat": False, "flat_duration": 0, "history": []}
...
def analyze_ema_expansion(close_prices):
    ...
    if len(close_prices) < 50:
        return {"ema20": None, "ema50": None, "ema200": None, "cross": "NONE", "is_expanding": False, "diff": 0}
...
def detect_rsi_divergence(candles, rsi_values):
    ...
    Finds swing lows and swing highs across price and RSI.
    """
    if len(candles) < 30 or len(rsi_values) < 30:
        return {"divergence": "NONE", "detail": ""}

    close_prices = [c["close"] for c in candles]
    # Align RSI list with candles
    rsi_offset = len(close_prices) - len(rsi_values)
    aligned_rsi = [50.0] * rsi_offset + rsi_values
    
    # Find Local Swing Lows (Bottoms)
    price_lows = []
    for i in range(len(close_prices) - 20, len(close_prices) - 2):
...
def calculate_rsi_tuetrading(candles):
    ...
    if len(close_prices) <= 25:
        return {"rsi": 50.0, "signal": 50.0, "status": "Trung tính 🟢", "divergence": "NONE", "squeeze_breakout": False}
...
def analyze_timeframe(candles):
    ...
    # Rating Score Calculation (0 to 100)
    score = 50
    reasons = []
    ...
    if ema_info["is_expanding"]:
        score += 10 if current_price > ema_info["ema20"] else -10
